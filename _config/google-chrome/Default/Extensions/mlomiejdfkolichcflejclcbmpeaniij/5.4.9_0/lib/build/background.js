...
define('lib/i18n',['underscore', 'lib/utils'], function (_, utils) {
...
define('tpl/click2play',['require','lib/i18n'],function(require){
...
require([
	...
	'lib/cmp',
	'tpl/click2play'
], function ($, _, bugDb, c2pDb, compDb, tagDb, conf, dispatcher, metrics, foundBugs, ghostrank, i18n, matcher, surrogatedb, tabInfo, utils, cmp, c2p_tpl) {

	var prefs = utils.prefs,
		log = utils.log,
		upgrade_alert_shown = false,
		walkthrough_shown_count = prefs('walkthrough_shown_count') || 0,
	...
	function injectedScriptMessageListener(name, message, tab_id, tab_url, sendResponse) {
		...
		if (name == 'openWalkthrough') {
			...
			openTab('https://www.ghostery.com/survey/in-app');
			...
			response = {
				...
				prefs: {
					...
					walkthrough_shown_count: prefs('walkthrough_shown_count')
				}
			};
		...
		} else if (name == "dismissCMPMessage") {
			CMP_DATA.splice(0, 1);
		}
		...
		utils.getActiveTab(function (tab) {
			...
			if (CMP_DATA.length !== 0) {
				...
				if (!conf.ghostrank && (prefs('walkthroughAborted') || prefs('walkthroughFinished')) &&
					walkthrough_shown_count < 3) {
					prefs('walkthrough_shown_count', ++walkthrough_shown_count);
	...
	chrome.runtime.getPlatformInfo(function (sysinfo) {
		chrome.runtime.setUninstallURL('https://d.ghostery.com/uninstall' +
			'?gr=' + (conf.ghostrank ? '1' : '0') +
			'&v=' + encodeURIComponent(utils.VERSION) +
			'&os=' + encodeURIComponent(sysinfo.os) +
			'&ua=ch');
...
define("lib/background", function(){});
